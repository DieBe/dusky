...
switch_text_editor() {
    ...
    if [[ ! -f "$CONF_VARS" ]]; then
        mkdir -p "$(dirname "$CONF_VARS")"
        atomic_write "$CONF_VARS" "\$textEditor = $target"
    fi
    ...
    if [[ ! -f "$CONF_BINDS" ]]; then
        mkdir -p "$(dirname "$CONF_BINDS")"
        atomic_write "$CONF_BINDS" "# Auto-generated by Text Editor Switcher"
    fi
...
main() {
    ...
    if [[ $# -eq 0 ]]; then
        ...
        case "$1" in
            ...
            --kate)              switch_text_editor "kate" ;;
            --code)              switch_text_editor "code" ;;
            --vscodium)          switch_text_editor "vscodium" ;;
            --zeditor)           switch_text_editor "zeditor" ;;
            --helix)             switch_text_editor "helix" ;;
            --emacs)             switch_text_editor "emacs" ;;
            --micro)             switch_text_editor "micro" ;;
            --set)
                if [[ -n "${2:-}" ]]; then
