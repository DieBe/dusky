...
switch_file_manager() {
    ...
    if [[ ! -f "$CONF_VARS" ]]; then
        ...
        atomic_write "$CONF_VARS" "\$fileManager = $target"
    fi
    ...
    if [[ ! -f "$CONF_BINDS" ]]; then
        ...
        atomic_write "$CONF_BINDS" "# Auto-generated by FM Switcher"
    fi
...
main() {
    ...
    if [[ $# -eq 0 ]]; then
        ...
        case "$1" in
            ...
            --dolphin) switch_file_manager "dolphin" ;;
            --nautilus) switch_file_manager "nautilus" ;;
            --pcmanfm) switch_file_manager "pcmanfm" ;;
            --ranger) switch_file_manager "ranger" ;;
            --lf) switch_file_manager "lf" ;;
            --superfile) switch_file_manager "superfile" ;;
            --set)
                if [[ -n "${2:-}" ]]; then
